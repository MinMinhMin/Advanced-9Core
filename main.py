...
class AddOptionMod(ctk.CTkFrame):
    def __init__(self, parent, text, column, row, type1, type2, type3, type4):
        ...
        self.option_var = ctk.StringVar(value='Default')
...
class timeframe(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.time_slider = ctk.CTkSlider(self, width=300, height=20,
                                         button_color='#f5bcf5', button_hover_color='#f294f2',
                                         variable=self.time_slider_var)

        self.label_time.grid(row=0, column=0)

        self.time_slider.grid(row=0, column=1, columnspan=3)

        self.place(relx=0.49, rely=0.8, anchor='center', relwidth=0.3, relheight=0.2)
...
class App(ttk.Window):
    ...
    def func1(self, event):
        self.timeframe.time = int((event.x / 300) * self.timeframe.end_time)
        self.timeframe.small_label.place_forget()
        self.timeframe.small_label.place(x=event.x + 20, y=event.y - 10, anchor='nw')
    ...
    def func3(self, event):
        self.timeframe.time = min(max(int((event.x / 300) * self.timeframe.end_time), 0), self.timeframe.end_time)
        self.timeframe.small_label.place_forget()
        self.timeframe.small_label.place(x=event.x + 20, y=event.y - 10, anchor='nw')
        self.timeframe.small_label.configure(
            text=f'{self.change_time(self.timeframe.time)}/{self.change_time(self.timeframe.end_time)}',
